...
public class ComputerPlayer extends Player {
	...
	public BoardCell pickLocation(Set<BoardCell> targets) {
		...
		if (b instanceof RoomCell) lastRoomVisited = b.getType();
		return b;
	}
	...
	public Solution createSuggestion(ArrayList<Card> deck,
			ArrayList<Card> unseen, Map<Character, String> key) {
		String accusedName = "";
		String accusedWeap = "";
		...
		Random rn = new Random();
		accusedName = playerChoices.get(
				Math.abs(rn.nextInt()) % playerChoices.size()).getName();
		accusedWeap = weapChoices.get(
				Math.abs(rn.nextInt()) % weapChoices.size()).getName();
	...
	public ComputerPlayer(String name, String color,
			BoardCell startingPosition, ArrayList<Card> deck) {
	...
	public void handleTurn(ClueGame clueGame) {
		...
		if (unseen.size() == 3) {
			...
			if (currentPosition.isRoom()) {
				createSuggestion(clueGame.getDeck(), unseen, board.getRooms());

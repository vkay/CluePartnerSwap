	...
	public void draw(Graphics g, Board board) {
		...
		if (isDoorway()) {
			...
			if (getDoorDirection() == DoorDirection.UP) {
				for (int i = 0; i < 4; i++)
					g.drawLine(getCol() * LENGTH, getRow() * LENGTH + i,
							getCol() * LENGTH + LENGTH, getRow() * LENGTH + i);
			} else if (getDoorDirection() == DoorDirection.DOWN) {
				for (int i = 0; i < 4; i++)
					g.drawLine((getCol()) * LENGTH,
							(getRow() + 1) * LENGTH - i, (getCol() + 1)
									* LENGTH, (getRow() + 1) * LENGTH - i);
			} else if (getDoorDirection() == DoorDirection.RIGHT) {
				for (int i = 0; i < 4; i++) {
					g.drawLine((getCol() + 1) * LENGTH - i,
							(getRow()) * LENGTH, (getCol() + 1) * LENGTH - i,
							(getRow() + 1) * LENGTH);
				}
			} else if (getDoorDirection() == DoorDirection.LEFT) {
				for (int i = 0; i < 4; i++) {
					g.drawLine((getCol()) * LENGTH + i, (getRow()) * LENGTH,
							(getCol()) * LENGTH + i, (getRow() + 1) * LENGTH);
				}
			}
		}
		if (getNameHere()) {
			g.setColor(Color.black);
			g.drawString(board.getRooms().get(getType()), getCol() * LENGTH,
					getRow() * LENGTH - 10);
		}
	}
}